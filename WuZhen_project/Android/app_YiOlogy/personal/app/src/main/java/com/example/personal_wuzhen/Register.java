...
public class Register extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            //选项被选中时的方法 参数分别为 spinner控件 被选中的项 选中项的位置 选中项的id
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                TextView text=(TextView)view;
                userSex=text.getText().toString();
            }
            @Override
            ...
            public void onClick(View v) {
                ...
                if(userName.length()<=0||userPw.length()<=0){
                    ...
                    if (userPw.equals(userPwConfirm)) {
                        ...
                        Toast.makeText(Register.this, "您已注册成功，请牢记用户名和密码,将在3秒后返回登陆界面"+userSex, Toast.LENGTH_LONG).show();
                        //倒计时函数
                        countdown();
                    } else {
    ...
    private void init() {
        ...
        spinner=findViewById(R.id.spinner_userSex);
        myDBHelper=new MyDBHelper(Register.this);//数据库类文件实例化
        db=myDBHelper.getWritableDatabase();//调用该方法便可以向表中添加数据
        ArrayAdapter<String> adapter=new ArrayAdapter<String>(Register.this,android.R.layout.simple_spinner_item,sex);
